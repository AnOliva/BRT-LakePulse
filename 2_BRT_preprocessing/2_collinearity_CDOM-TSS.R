...
subDir <- "multicollinearity_QC"
ifelse(!dir.exists(file.path(subDir)), dir.create(file.path(subDir)), FALSE)

tiff(filename = paste0("multicollinearity_QC/multicollinearity_pearson_CDOM-TSS.tif"), width = 95, height = 95, units = "mm", res = 300)
par(mar=c(0.5, 0.5, 0.5, 0.5))
corrplot(cor(data.merged[ , ! names(data.merged) %in% c("ecozone")],
...
indpt_var = data_randomized[ , ! names(data.merged) %in% c("TSS","aCDOM400")]
# species variables
dpt_var = data_randomized[ , c("TSS","aCDOM400")] 
...
for(i in 1:ncol(dpt_var)) {
  ...
  indpt_var2 = indpt_var[!is.na(indpt_var$dptvar) ,]
  
  # glm model
  modeltest = glm(dptvar ~ ., data= indpt_var2, family = gaussian(link = "identity"))
...
# print VGIF
round(unlist(lapply(list_vif, function(x) x[,3][which.max(abs(x[,3]))])),2)
